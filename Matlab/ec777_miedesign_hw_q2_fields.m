...
% TM
agnk = gaas_spline(wv);
[~, scatter_max_index] = max(scatter_q_tm(agnk, x, nmodes));
x_resonance = x(scatter_max_index);
k = 2*pi/wv(scatter_max_index);
rix_resonance = agnk(scatter_max_index);
...
fprintf('Figure 1: EM fields for Ag scattering at wavelength = %0.3f microns.\n', wv(scatter_max_index));
%% Si nanowires.
% TM
aunk = si_spline(wv);
[~, scatter_max_index] = max(scatter_q_tm(aunk, x, nmodes));
x_resonance = x(scatter_max_index);
k = 2*pi/wv(scatter_max_index);
rix_resonance = aunk(scatter_max_index);
...
fprintf('Figure 1: EM fields for Au scattering at wavelength = %0.3f microns.\n', wv(scatter_max_index));
